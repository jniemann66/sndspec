...
struct Marker
{
	...
	std::string displayText() const
	{
		...
		oss << std::fixed << std::setprecision(1) << freq /*<< " Hz"*/;
		return oss.str();
	}
};
...
public:
	...
	// plot the actual window function
	void renderWindowFunction(const Parameters& parameters, const std::vector<double> &data);
